...
const COMPOSER_BUTTON_SELECTOR = "form button";
...
export function createChatGptInteractiveNodeBudgetSnapshot(
    ...
    const threadButtons = uniqueElements(turns.flatMap((turn) => query(turn, BUTTON_SELECTOR)));
    const threadSvgs = uniqueElements(turns.flatMap((turn) => query(turn, SVG_SELECTOR)));
    const toolGroups = uniqueElements(turns.flatMap((turn) => query(turn, CHATGPT_TOOL_SELECTOR)));
