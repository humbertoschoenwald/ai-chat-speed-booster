...
function chatGptNode(role: "user" | "assistant" | "tool", parent: string, child?: string) {
    return {
        message: {
            author: { role },
...
test("ChatGPT Fast Mode does not let tool nodes crowd out recent messages", async ({ page }) => {
    const mapping = {
        root: { message: null, parent: null, children: ["n1"] },
        n1: chatGptNode("user", "root", "n2"),
        n2: chatGptNode("assistant", "n1", "n3"),
        n3: chatGptNode("tool", "n2", "n4"),
        n4: chatGptNode("tool", "n3", "n5"),
        n5: chatGptNode("user", "n4", "n6"),
        n6: chatGptNode("assistant", "n5", "n7"),
        n7: chatGptNode("tool", "n6", "n8"),
        n8: chatGptNode("tool", "n7"),
    };

    await page.route("https://chatgpt.com/c/mock-tool-crowding", (route) =>
        route.fulfill({ contentType: "text/html", body: "<main>mock ChatGPT</main>" }),
    );
    await page.route("https://chatgpt.com/backend-api/conversation/mock-tool-crowding", (route) =>
        route.fulfill({
            contentType: "application/json",
            body: JSON.stringify({ mapping, current_node: "n8", root: "root" }),
        }),
    );

    await page.goto("https://chatgpt.com/c/mock-tool-crowding", { waitUntil: "domcontentloaded" });
    await page.waitForFunction(() => Boolean((window as unknown as Record<string, unknown>).__ACSB_FETCH_PATCHED__));
    await page.evaluate(() => {
        localStorage.setItem("acsb_bridge_config", JSON.stringify({
            enabled: true,
    ...
    const trimmed = await page.evaluate(async () => {
        const response = await fetch("/backend-api/conversation/mock-tool-crowding");
        return response.json();
    }) as { mapping: Record<string, { parent: string | null; children: string[] }> };
    ...
    expect(trimmed.mapping.n5.parent).toBe("root");
    expect(trimmed.mapping.n5.children).toEqual(["n6"]);
    expect(trimmed.mapping.n6.children).toEqual(["n7"]);
    expect(trimmed.mapping.n7.children).toEqual(["n8"]);
});
